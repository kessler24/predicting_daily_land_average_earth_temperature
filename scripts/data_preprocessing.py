"""
Preprocessing pipeline for global temperature anomaly data.

This script loads the raw global temperature anomaly dataset, performs cleaning,
feature engineering, validation (schema and correlation), and splits the data into training and test sets. The cleaned datasets, validation logs, and plots are saved to user-specified directories.

The preprocessing pipeline includes:
- Reading raw data from CSV
- Cleaning fields and generating derived variables
- Running Pandera schema validation on all data
- Running additional validation checks on training data only
- Running DeepChecks correlation validations on training data
- Saving cleaned train and test datasets

Parameters
----------
read_path : str
    Path to the raw CSV file to be processed.
write_path : str
    Directory where training and test datasets will be saved as CSVs.
plots_path : str
    Directory where generated plots will be saved as PNGs.
logs_path : str
    Directory where validation logs will be written as .log files.
...
Notes
-----
This script is designed as a preprocessing step before exploratory data analysis and model training. If any data validation checks fail, this script may throw warnings or errors.

This script throws a DeprecationWarning for pkg_resources used by DeepChecks. Developers have investigated this warning and decided it is not urgent to address at this time.
"""
...
def main(read_path, write_path, plots_path, logs_path):
    ...
    This function coordinates loading the dataset, cleaning, schema validation, 
    correlation validation, data splitting, and saving outputs.
    It is executed when the script is run from the command line.

    Parameters
    ----------
    read_path : str
        Path to the raw global temperature anomaly CSV file.
    write_path : str
        Output prefix used to generate cleaned train and test CSVs.
    plots_path : str
        Directory where visualization files will be stored.
    logs_path : str
        Directory where validation logs will be written.
...
def save_altair_chart(chart, filename, folder, scale_factor=2.0):
    ...
    ----------
    chart : alt.Chart
        The Altair chart object to be saved.
    filename : str
        Name of the output image file (e.g., "plot.png").
    folder : str
        Destination folder where the output file will be saved.
    scale_factor : float, optional
        Scaling factor for increasing chart resolution (default = 2.0).
...
def data_cleaning(raw_df, BASELINE_TEMP = 8.59):
    """
    Clean and modify the raw temperature anomaly dataset.

    Performs column removal, calculates actual temperatures from anomaly column,
    generates month name labels, and ensures an ordered categorical
    representation for month names.

    Parameters
    ----------
    raw_df : pd.DataFrame
        Raw input DataFrame loaded from the global anomaly dataset.
    BASELINE_TEMP : float, optional
        Baseline temperature added to anomalies to recover absolute temperature.
        Default is 8.59°C as specified in data source, representing the global 
        land-average temperature in Celsius from January 1951 - December 1980.
...
def all_data_validation(df):
    """
    Validate the full dataset using a Pandera schema.

    Ensures all required columns exist, have correct data types, fall within
    acceptable value ranges, contain no duplicates, and match strict structure
    expectations. Logs validation errors when schema checks fail.

    Parameters
    ----------
    df : pd.DataFrame
        Cleaned DataFrame containing all daily records.

    Returns
    -------
    pd.DataFrame
        Validated DataFrame (unchanged if validation passes).

    Notes
    -----
    This validation step applies to the entire dataset, including both
...
def split_data(df, cutoff = 2012):
    """
    Split the dataset into training and test sets by year.

    Parameters
    ----------
    df : pd.DataFrame
        Validated temperature anomaly dataset.
    cutoff : int, optional
        Year cutoff used for splitting. Data with `Year <= cutoff` becomes
        the training set; later years form the test set. Default is 2012.
...
def visualize_distribution(train_df, plots_path):
    """
    Generate histograms of Temperature and Anomaly for the training dataset.

    Creates Altair visualizations showing distributional shape, overlaid
    with mean and median reference lines and labels. Saves the resulting
    plots side-by-side.
    ...
    plots_path : str
        Destination directory for saving visualization images.
...
def training_data_validation(train_df):
    """
    Apply training-only data validation using a Pandera schema.

    This schema enforces distributional assumptions about the
    target variables (Temperature and Anomaly). Validation failures are logged.

    Parameters
    ----------
    train_df : pd.DataFrame
        Training dataset in which target distribution assumptions must hold.
    ...
    Notes
    -----
    Test data is intentionally excluded from these checks to avoid leakage of
...
def correlation_validation(train_df, plots_path):
    """
    Perform correlation analysis and visualization on the training dataset.

    Computes a Pearson correlation matrix, visualizes it as a heatmap,
    runs DeepChecks feature-label and feature-feature correlation tests,
    and generates an explanatory scatterplot for correlated calendar features.

    Parameters
    ----------
    train_df : pd.DataFrame
        Training dataset used for correlation inspection.
    plots_path : str
        Directory where correlation plots will be saved.
    ...
    Notes
    -----
    - DeepChecks conditions warn if correlations exceed acceptable thresholds.
    - Only training data is used to avoid leaking statistical relationships
      into the test set.
    """
