...
This project analyzed Berkeley Earth global daily average land-surface temperature data from 1880–2022. The goal was to clean, explore, and model this historical data to predict future temperature trends. After evaluating three regression approaches: Linear Regression, Random Forest, and Support Vector Regression (SVR),the SVR model performed best and was used to forecast the land average temperature for the year 2030. The findings are important for climate risk assessment and informing mitigation policy.

However, limitation of the model exists as the model is built on global historical temperature data without incorporating any external features (such as Carbon dioxide concentrations) and regional data. Future work can focus on feature engineering by integrating more relevant features and develop regional forecasts.

# Introduction

Understanding long-term changes in global land temperature is critical for studying and responding to the escalating global crisis of climate change. Daily temperature anomaly data collected over more than 140 years provide an opportunity to model how temperatures have shifted, identify crucial long-term patterns, and forecast future warming. The urgency of this analysis is underscored by the severe, tangible impacts of rising temperatures, which threaten global food security, overwhelm infrastructure with increased extreme weather events (e.g., droughts, severe storms), and accelerate biodiversity loss.

In this report, we discuss both about the actual temperature and the temperature anomaly. According to @NCEI_anomalies, temperature anomaly refers to the deviation from a baseline climatological temperature. In this study, the baseline is 8.59°C, representing the January 1951–December 1980 global land-average temperature. By introducing anomalies, we can effectively normalize the data across the time periods, allowing us to highlight the long-term warming trend that defines climate change.
...
The ability to accurately forecast the global land-average temperature for a near-term year like 2030, moves the conversation beyond historical observation and into proactive decision-making and risk management. The prediction can be meaningful for several aspects such as assisting policy evaluation in targeting emission reduction, adaptation planning for relevant infrastructure, and raising public awareness to appeal for early actions.
...
The project methodology began with rigorous **data validation** of the 1880–2012 daily temperature data using @pandera to enforce structural integrity, detect outliers, and ensure data quality. Following validation, the data was resampled to **annual mean anomaly values** to focus on long-term climate trends, and then split into training (pre-2012) and test (post-2012) sets for future prediction evaluation. **Exploratory Data Analysis (EDA)** confirmed a linear increasing temperature trend over time. Linear Regression, Random Forest, and Support Vector Regression (SVR) models were trained, and the **SVR model** was ultimately selected for its superior performance (lowest **RMSE** of 0.144) on the unseen test data, which was subsequently used to forecast the global land-average temperature for the year 2030.
...
![Visualize the raw data and mean per year for temperature over time.](../images/eda_mean_per_year_plot.png){#fig-scatter}
...
![Visualize the mean per year aggregated data with a linear fit on the training dataset.](../images/eda_linear_fit_plot.png){#fig-reg}
...
![Visualize the mean temperature per year grouped by month.](../images/eda_facet_by_month_plot.png){#fig-facet}
